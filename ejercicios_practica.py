...
def fill():
    ...
    tutor = Tutor(name='guillermo')
    session.add(tutor)
    tutor = Tutor(name='nelson')
    session.add(tutor)
    tutor = Tutor(name='alejandra')
    session.add(tutor)
    tutor = Tutor(name='pedro')
    session.add(tutor)
    session.commit()
    ...
    estudiante = Estudiante(name='natalia', age=14, grade=2, tutor_id=1)
    session.add(estudiante)
    estudiante = Estudiante(name='micaela', age=12, grade=1, tutor_id=1)
    session.add(estudiante)
    estudiante = Estudiante(name='pepito', age=19, grade=6, tutor_id=1)
    session.add(estudiante)
    estudiante = Estudiante(name='juan', age=19, grade=3, tutor_id=2)
    session.add(estudiante)
    estudiante = Estudiante(name='fulanito', age=17, grade=4, tutor_id=1)
    session.add(estudiante)
    estudiante = Estudiante(name='menganito', age=15, grade=4, tutor_id=2)
    session.add(estudiante)
    estudiante = Estudiante(name='firulais', age=17, grade=5, tutor_id=3)
    session.add(estudiante)
    session.commit()
...
def modify(id, name):
    ...
    update = session.query(Tutor).filter(Tutor.name == name)
    tutor = update.first()
       
    update = session.query(Estudiante).filter(Estudiante.id == id)
    estudiante = update.first()
    estudiante.tutor_id = tutor.id
    
    session.add(estudiante)
    session.commit()
    print('Contenido modificado')
    # Vuelvo a imprimir la tabla para ver el contenido modificado
    fetch()
...
def count_grade(grade):
    ...
    print('cantidad de estudiantes por grado', grade ,'es', contador)
